...
# ==============================================================================
# 【全能修复补丁】: 强行替换 wandb，防止 losses.py 等其他文件报错
# ==============================================================================
class MockWandb:
    """一个假的 WandB 类，吃掉所有调用，什么都不做"""
    ...
    # 各种数据类型占位符
    def Object3D(self, *args, **kwargs): return "DummyObject3D"

    def Html(self, *args, **kwargs): return "DummyHtml"

    def Table(self, *args, **kwargs): return "DummyTable"

    def Image(self, *args, **kwargs): return "DummyImage"  # <--- 刚才缺的就是这个

    def Video(self, *args, **kwargs): return "DummyVideo"

    def Audio(self, *args, **kwargs): return "DummyAudio"
...
# 这一步最关键：在其他文件 import wandb 之前，先把系统里的 wandb 换成假的
# 这样 hit/training/losses.py 导入的 wandb 也是这个假的
...
import yaml
import wandb  # 这里导入的已经是 MockWandb 了
...
# 注意：losses 必须在 sys.modules['wandb'] 被替换后导入
...
class HITTraining(pl.LightningModule):
    ...
    # 【修复 1】：添加 save_dir 属性，防止没有 logger 时找不到路径
    @property
    def save_dir(self):
        # 如果 logger 存在且有 save_dir，就用它
        if self.logger and hasattr(self.logger, 'save_dir') and self.logger.save_dir:
            return self.logger.save_dir
        # 否则回退到默认路径
        return os.path.join(cg.trained_models_folder, self.cfg.exp_name)
        ...
        if self.train_cfg['to_train'] == 'occ':
            ...
            if self.train_cfg['forward_beta_mlp']:
                params_to_optimize.append(
                    {'params': self.hit.fwd_beta.parameters(), 'weight_decay': self.cfg['weight_decay_fwd_beta']})
            if self.train_cfg['optimize_occ']:
                params_to_optimize.append({'params': self.hit.tissues_occ.parameters()})
                # ==== 追加这几行，让优化器更新我们的点云特征网络 ====
                if hasattr(self.hit, 'use_surface_feats') and self.hit.use_surface_feats:
                    params_to_optimize.append({'params': self.hit.surface_encoder.parameters()})
            ...
            if self.train_cfg['optimize_compressor'] and self.train_cfg['compressor']:
                params_to_optimize.append({'params': self.hit.deformer.compressor.parameters(),
            ...
            params_to_optimize.append(
                {'params': self.hit.deformer.disp_network.parameters(), 'weight_decay': self.cfg['weight_decay_disp']})
            params_to_optimize.append(
                {'params': self.hit.deformer.lbs_network.parameters(), 'weight_decay': self.cfg['weight_decay_lbs']})
            if self.train_cfg['pose_bs']:
                params_to_optimize.append({'params': self.hit.deformer.pose_bs.parameters()})
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(
            MRIDataset.from_config(self.smpl_cfg, self.data_cfg, self.train_cfg, split='train'), drop_last=True,
    ...
    def val_dataloader(self):
        ...
        return torch.utils.data.DataLoader(mri_dataset, drop_last=True,
                                           num_workers=self.cfg.num_workers, batch_size=self.cfg['batch_size'],
                                           pin_memory=False, shuffle=False)

    def test_dataloader(self):
        return torch.utils.data.DataLoader(
            MRIDataset.from_config(self.smpl_cfg, self.data_cfg, self.train_cfg, split='test'), drop_last=True,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.train_cfg['lambda_compression'] > 0 and self.train_cfg['compressor'] and self.train_cfg[
            'optimize_compressor']:
        ...
        if self.train_cfg['lambda_comp0'] > 0 and self.train_cfg['compressor'] and self.train_cfg[
            'optimize_compressor']:
    ...
    def on_before_optimizer_step(self, optimizer, optimizer_idx):
        if self.cfg.print_grad_clip:
            ...
            max_grad = max(
                [param.grad.abs().max() for name, param in self.named_parameters() if param.grad is not None])
    ...
    def compute_test_loss(self, batch):
        ...
        for li, mri_label in enumerate(['NO', 'LT', 'AT', 'BONE']):
            ...
            test_loss_dict[f"comp_{mri_label}"] = metrics.compute_composition(pred_occ, gt_occ, batch['body_mask'],
                                                                              mri_shape=mri_shape)
            ...
            # 【修复 2】：使用 self.save_dir
            dst_slices_dir = os.path.join(self.save_dir, 'test', 'slices',
                                          f'epoch={self.current_epoch:05d}_step={self.global_step:05d}', mri_label)

            mri_uvs = batch['mri_coords']
            batch_images = metrics.per_slice_prediction(pred_occ, gt_occ, mri_uvs, mri_shape, dst_slices_dir,
                                                        batch['seq_names'], level=0.5, return_image=True)
            ...
            for idx in range(d):
                ...
                for li, mri_label in enumerate(['NO', 'LT', 'AT', 'BONE']):
                    if mri_label == 'NO':
                        continue
                    # import ipdb; ipdb.set_trace()
                    composite_gt += np.repeat(images_dict[li][b][idx]['gt'].astype(int).reshape(h, w, 1), 3, axis=-1) * \
                                    palette[li]
                    composite_pred += np.repeat(images_dict[li][b][idx]['pred'].astype(int).reshape(h, w, 1), 3,
                                                axis=-1) * palette[li]
                    # Compute differences
                    # 【修复 2】：使用 self.save_dir
                    dst_slices_dir = os.path.join(self.save_dir, 'test', 'slices',
                ...
                img = (
                    np.where(composite_gt == np.array([0., 0., 0.]), 2, composite_gt == composite_pred).all(-1)).astype(
                    np.uint8)
    ...
    def test_epoch_end(self, outputs):
        ...
        # 【修复 2】：使用 self.save_dir
        with open(self.save_dir + '/part_metric.pkl', 'wb') as fp:
        ...
        # 【修复 2】：使用 self.save_dir
        test_res_table_path = os.path.join(self.save_dir, f'test_persubject_table.txt')
        ...
        val_metric_print = {key: [float(val.item() if torch.is_tensor(val) else val)] for key, val in
                            agg_metric.items()}
        x = PrettyTable()
        ...
        # Log to a file
        # 【修复 2】：使用 self.save_dir
        test_res_table_path = os.path.join(self.save_dir, f'test_mean_table.txt')
        with open(test_res_table_path, 'w') as w:
        ...
        if self.trainer.is_global_zero:  # to avoid deadlock
            ...
            # 【修复 2】：使用 self.save_dir
            dst_path = os.path.join(self.save_dir,
...
def main(cfg):
    # 【修复 4】：初始化 resuming_run 防止 UnboundLocalError
    resuming_run = False
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(dirpath=f'{out_dir}/ckpts/',
                                                       ...
                                                       monitor=ckp_monitor,
                                                       mode=mode,
                                                       # The best model is the one with the highest val/accuracy
    ...
    if cg.logger == 'wandb':
        # resuming_run = False # Moved to top
        if cfg.wdboff:
            wandbmode = 'disabled'
        else:
            wandbmode = 'online'
        ...
        if cfg.resume and len(glob.glob(os.path.join(out_dir, 'wandb', 'run*'))) > 0:
            ...
            resuming_run = True
            logger = pl.loggers.WandbLogger(name=cfg.exp_name, entity=cg.wandb_entity, project=cg.wandb_project_name,
                                            id=id, save_dir=out_dir, log_model=False, resume='must', mode=wandbmode)
        else:
            logger = pl.loggers.WandbLogger(name=cfg.exp_name, entity=cg.wandb_entity, project=cg.wandb_project_name,
                                            save_dir=out_dir, log_model=False, mode=wandbmode)
    else:
        # 【修复 3】：禁用 TensorBoardLogger，改为 False
    ...
    # 【修复 5】：给 logger.experiment 加安全判断
    if not resuming_run and logger and hasattr(logger, 'experiment'):
